...
- 标注是猫检测框中心对应的可见表面估计，不是猫的脚底或导航目标；不自动驱动车体或云台。
...
地图位置包含两次旋转：**云台实测 yaw → 车体在 map 中的姿态**。使用图像接收时采样的云台 USB 反馈角度，不使用滑条设定角；云台断开、反馈过期或有阻断故障时不标注。当前 `follow_pitch: false`，按水平相机近似，暂不叠加 Pitch；需要三维俯仰补偿时，确认 pitch 正方向并配置 `pitch_sign`，再启用 `follow_pitch`。
...
| `pitch_xyz_m` | pitch 轴心相对 yaw 轴心的偏移，在 yaw 零位坐标系中表示 |
| `camera_xyz_m` | 相机光学中心相对 pitch 轴心的偏移，在 pitch 零位机械坐标系中表示 |
| `camera_rpy_deg` | 相机光学坐标系相对 pitch 零位机械坐标系的旋转；若镜头正前、图像上方朝上，常见值为 [-90, 0, -90]，须按实物确认 |
| `yaw_sign` | 编码器正向与绕 +Z 右手旋转的关系：+1 或 -1 |
| `pitch_sign` | 编码器正向与绕 +Y 右手旋转的关系：+1 或 -1（绕 +Y 正转是低头） |
| `follow_pitch` | 是否叠加实测 Pitch，当前为 false；Yaw 始终叠加 |
| `map_frame` | 默认 map，需要导航或 SLAM 发布地图 TF |

变换顺序为：安装基准 → yaw 旋转 → pitch 轴心平移 → pitch 旋转 → 相机平移与光学旋转。使用接收图像时刻采样的云台实测角度，不用滑条目标代替反馈。云台反馈断开或有电机故障时停止标注。
