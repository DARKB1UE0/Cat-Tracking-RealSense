...
截至 2026-09-17，已编译、模拟验证并通过 ST-Link 烧录和校验零位保持固件。yaw 安装零位按现有左转为正约定，相对电机原点顺时针偏移 90°；pitch 使用电机原点。实际安装方向与正前方对准精度、ROS/USB 完整链路仍需现场验证。
...
| 类型 | payload | 行为 |
|------|---------|------|
| `0x01` SETPOINT | `<ffBBH>`，12 字节 | 相对所配置安装零点的 yaw、pitch（度），mode（0 锁存停止 / 1 位置控制），flags=0，reserved=0 |
...
开机未收到上位机指令，或最近的运动指令超过 100 ms 未更新时，下位机默认以 yaw=0°、pitch=0° 进行位置闭环，持续向两轴发送 CAN 控制帧，回到配置的安装零点并保持。失效的非零目标不会继续执行。默认回零沿用位置/速度环的输出限幅，实际到位精度和负载保持能力需调试 PID 验证。
...
| yaw 安装零位 | `GIMBAL_YAW_ZERO_DEG=-90°`，等价原始角 270°、编码器约 6144；按当前方向约定顺时针偏移 90° |
| pitch 零位 | DM4310P 反馈位置 0 rad |
...
用户反馈原 yaw 零点朝向车体左侧，因此增加基于电机原点的安装偏移：`GIMBAL_YAW_ZERO_DEG=-90.0f`。按当前 `GIMBAL_YAW_SIGN=+1` 且左转为正的约定，顺时针 90° 对应原始编码器角 -90°，等价 270°（约 6144）。该方向关系需按实际安装确认，代码不会自动识别顺/逆时针。

初始化按 `零位 + round((当前角度 - 零位) / 360) × 360` 选择最近一圈，随后固定基准并跟踪跨圈变化。例如原始角 0° 时目标为 -90°，原始角 355° 时目标为 270°，避免多绕一圈。yaw=0 的指令、默认保持、相对限位与反馈使用同一安装零位，pitch 仍为达妙反馈位置 0 rad。首次有效反馈仅用于选取圈数，不将开机姿态重新设为零，也不改写电机内部编码器原点。

USB 指令和状态角度均相对上述安装零点，内部闭环统一使用度和度/秒。如果新的 yaw=0 已对准车头，上位机无需再叠加同一个 90° 修正，相机 TF 的 `yaw_offset_deg` 只填写剩余安装偏差。`GIMBAL_YAW_SIGN`、`GIMBAL_PITCH_SIGN` 取 ±1，分别校准左转为正、抬头为正。`GimbalUsbSetInitialPose()` 是固件内部坐标接口，当前 USB 协议没有远程写零位命令。
...
`/gimbal/status` 数组顺序是 `[yaw_deg, pitch_deg, enabled, fault, age_ms]`。停止目标发布进程后，超过 100 ms 固件回到配置的安装零点并保持。若需要失能停机，先停止持续发布，再明确发送：
...
认定完整实物链路通过，需要观察到：USB 实际状态回传、CAN1 命令及两轴反馈、实测角度随小目标变化、未连接上位机时回到所配置零点、停止发布后回零保持、显式 STOP 后持续失能，以及断开任一电机反馈后两轴停止。用户已现场反馈旧零位朝左，本次已完成修正固件的烧录；上述完整联调项目尚未全部验证。
