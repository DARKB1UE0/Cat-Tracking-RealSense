...
截至 2026-09-17，软件控制链路已支持无上位机指令时回到电机零点并保持，ARM 固件编译和软件模拟通过。尚未烧录、运行真实 ROS 2 或连接 USB/CAN 电机联调。
...
认定实物链路通过，需要同时观察到：USB 实际状态回传、CAN1 命令及两轴反馈、实测角度随小目标变化、未连接上位机时回到电机零点、停止发布后回零保持、显式 STOP 后持续失能，以及断开任一电机反馈后两轴停止。目前这些硬件步骤尚未执行。
